...
phi_bk = torch.ones((Gaussian_Masks_torch.shape[0], Ny, Nx), dtype=torch.double, device=DEVICE) - torch.sum(Gaussian_Masks_torch, dim = 1)
...
for l in range(Gaussian_Masks_torch.shape[0]):
    for i in range(n_of_modes):
        phi_cr[l,i,:,:] = torch.sum(Gaussian_Masks_torch[l], dim = 0) - Gaussian_Masks_torch[l,i,:,:]
